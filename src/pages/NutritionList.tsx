...
import {
  CreateDessertVariables,
  CreateDessert,
} from "./__generated__/CreateDessert";
import {
  DeleteDesserts,
  DeleteDessertsVariables,
} from "./__generated__/DeleteDesserts";

export const DESSERT_DETAILS = gql`
...
const NutritionList: FC = () => {
  ...
  const { modal, selections } = state;

  const {
    data,
    loading: dessertListLoading,
    error: dessertListError,
    refetch: dessertListRefetch,
  } = useQuery<dessertList>(GET_DESSERTS, { fetchPolicy: "cache-and-network" });

  const [
    createDessert,
    { loading: createDessertLoading, error: createDessertError },
  ] = useMutation<CreateDessert, CreateDessertVariables>(CREATE_DESSERT);
  // , {
  ...
  const [
    deleteDessert,
    { loading: deleteDessertsLoading, error: deleteDessertsError },
  ] = useMutation<DeleteDesserts, DeleteDessertsVariables>(DELETE_DESSERTS);
  const [
    resetDessert,
    { loading: resetDessertLoading, error: resetDessertError },
  ] = useMutation<ResetDesserts>(RESET_DESSERTS, {
    onCompleted: (data) => {
      if (data) {
  ...
  if (modal) {
    return (
      <div className="center">
        <NewDessert
          onAddDessert={onSubmitNewDessert}
  ...
  const loading =
    dessertListLoading ||
    resetDessertLoading ||
    createDessertLoading ||
    deleteDessertsLoading;
    ...
    <>
      ...
      <div className="ph2 pv4">
        <ControlBox
          ...
          onDeleteDesserts={onDeleteDesserts}
        />
        {loading && <Loading />}
        {!loading && <DessertGrid />}
      </div>
